const ProductWrapper = ({isPartnerOpen,isBusinessOpen,setIsBusinessOpen,setIsPartnerOpen,isProductOpen,isOpen}) => {
    return(
               <div className={`grid gap-10  absolute bg-white inset-x-[10px] sm:inset-x-1/4 ${isPartnerOpen || isBusinessOpen ? 'sm:w-3/4 grid-cols-2' : 'sm:w-2/4 grid-cols-1'} top-20 z-[1000] px-7  ${isProductOpen && isOpen == false ? 'z-[1000] opacity-100' : '-z-50 opacity-0'}`}>
